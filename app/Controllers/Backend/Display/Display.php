<?php 
namespace App\Controllers\Backend\Display;
use App\Controllers\BaseController;
use App\Libraries\Nestedset_translatebie;
...
class Display extends BaseController{
	...
	public function __construct(){
		...
		$this->data['module2'] = 'display_catalogue';
		$this->nestedsetbie = new Nestedset_translatebie(['table' => $this->data['module'].'_catalogue','language' => $this->currentLanguage()]);
	...
	public function index($page = 1){
		...
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/display/display/index'
		]);
		if($flag == false){
		...
		if($config['total_rows'] > 0){
			$config = pagination_config_bt(['url' => 'backend/display/display/index','perpage' => $perpage], $config);
		...
		$this->data['template'] = 'backend/display/display/index';
	...
	public function create(){
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($resultid > 0){
	 				...
	 				if($flag > 0){
 						...
 						return redirect()->to(BASE_URL.'backend/display/display/index');
	 				}else{
		...
		$this->data['template'] = 'backend/display/display/create';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function update($id = 0){
		...
		$this->data[$this->data['module']] = $this->AutoloadModel->_get_where([
			'select' => 'tb1.id,tb1.price,tb1.price_promotion, tb4.title, tb4.link,tb4.canonical, tb4.description, tb4.content, tb4.meta_title, tb4.meta_description, tb1.catalogueid, tb1.image, tb1.album, tb1.publish, tb1.catalogue',
			...
			'join' => [
					...
					[
						'display_translate as tb4','tb1.id = tb4.objectid AND tb4.module = "display" AND tb4.language = \''.$this->currentLanguage().'\' ','inner'
					]
				],
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($flag > 0){
		 			$flagLang = $this->AutoloadModel->_update([
			 			...
			 			'where' => ['objectid' => $id, 'module' => 'display', 'language' => $this->currentLanguage()],
			 			'data' => $updateLanguage,
			 		]);
		...
		$this->data['template'] = 'backend/display/display/update';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function delete($id = 0){
		...
		$this->data['template'] = 'backend/display/display/delete';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function condition_catalogue(){
		...
		if($catalogueid > 0){
			$catalogue = $this->AutoloadModel->_get_where([
				...
				'join' =>  [
					[
						'display_translate as tb4','tb1.id = tb4.objectid AND tb4.module="display_catalogue" AND tb4.language = \''.$this->currentLanguage().'\' ','inner'
					],
									],
	...
	private function detect_language(){
		...
		if(isset($languageList) && is_array($languageList) && count($languageList)){
			foreach($languageList as $key => $val){
				$select = $select.'(SELECT COUNT(objectid) FROM display_translate WHERE display_translate.objectid = tb1.id AND display_translate.module = "display" AND  display_translate.language = "'.$val['canonical'].'") as '.$val['canonical'].'_detect, ';
				$i++;
			}	
		}
	...
	private function validation(){
		$validate = [
			...
			'link' => 'check_link['.$this->data['module'].']',
		...
		$errorValidate = [
			...
			'link' => [
				'check_link' => 'Đường dẫn đã tồn tại, vui lòng chọn đường dẫn khác',
			],
			'catalogueid' => [
