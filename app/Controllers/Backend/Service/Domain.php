...
class Domain extends BaseController{
	...
	public function __construct(){
		$this->data = [];		
		$this->data['module'] = 'domain';
	}

	public function index($page = 1){
		$session = session();
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/service/domain/index'
		]);
		...
		if($config['total_rows'] > 0){
			$config = pagination_config_bt(['url' => 'backend/service/domain/index','perpage' => $perpage], $config);
			...
			$this->data['domainList'] = $this->AutoloadModel->_get_where([
				'select' => 'id, title, price_sell, price_buy, price_extend, userid_created, userid_updated, publish, (SELECT fullname FROM user WHERE user.id = domain.userid_created) as creator, created_at, updated_at',
				'table' => $this->data['module'],
				'where' => $where,
				'keyword' => $keyword,
		...
		$this->data['template'] = 'backend/service/domain/index';
		...
		if($this->request->getMethod() == 'post'){
			...
			if ($this->validate($validate, $errorValidate)){
		 		...
		 		if($insertid > 0){
		 			...
		 			return redirect()->to(BASE_URL.'backend/service/domain/index');
		 		}
	        }else{
	        	$this->data['validate'] = $this->validator->listErrors();
	        }
		}
		$this->data['template'] = 'backend/service/domain/store';
	...
	public function update($id = 0){
		...
		$this->data[$this->data['module']] = $this->AutoloadModel->_get_where([
			'select' => 'id, title, price_sell, price_buy, price_extend',
			'table' => $this->data['module'],
	...
	public function delete($id = 0){
		...
		$this->data['template'] = 'backend/service/domain/delete';
		return view('backend/dashboard/layout/home', $this->data);
	}
