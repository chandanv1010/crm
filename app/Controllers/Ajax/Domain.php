...
class Domain extends BaseController{
	...
	public function save(){
		try{
			...
			$flag = $this->authentication->check_permission([
				'routes' => 'backend/service/domain/create'
			]);
			if($flag == false){
	...
	private function store(array $param = []){
		...
		$store['price_buy'] = str_replace('.', '', $this->request->getPost('price_buy'));
		$store['price_sell'] = str_replace('.', '', $this->request->getPost('price_sell'));
		$store['price_extend'] = str_replace('.', '', $this->request->getPost('price_extend'));
	...
	private function validation(){
		...
		$errorValidate = [
			'title' => [
				'required' => 'Bạn bắt buộc phải nhập vào tên Domain!',
			],
		];
		return [
			'validate' => $validate,
