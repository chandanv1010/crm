				...
				$method = ($id > 0) ? 'update' : 'create';
		 		...
		 		if($flag > 0){
					...
					$this->create_contract_detail($primary_id);
			 		...
			 		if(isset($save['money_default']) && $save['money_default'] == 1){

			 			$sum  = 0;
				 		$billing = $this->request->getPost('billing');
				 		if(isset($billing) && is_array($billing) && count($billing)){
					 		foreach ($billing['billingPrice'] as $key => $value) {
					 			$price  = convertMoney($value);
					 			$sum = $sum + $price;
					 		}
				 		}
			 			$save['method'] = $method;
			 			...
			 			$save['id'] = $primary_id;
			 			$save['catalogueid'] = 6;
			 			$save['date_start'] = $save['date_sign'];
