...
class Catalogue extends BaseController{
	...
	public function __construct(){
		$this->data = [];
		$this->data['module'] = 'display_catalogue';
		$this->nestedsetbie = new Nestedset_translatebie(['table' => $this->data['module'],'language' => $this->currentLanguage()]);
	...
	public function index($page = 1){
		...
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/display/catalogue/index'
		]);
		if($flag == false){
		...
		if($config['total_rows'] > 0){
			$config = pagination_config_bt(['url' => 'backend/display/catalogue/index','perpage' => $perpage], $config);
			...
			$this->data['displayCatalogueList'] = $this->AutoloadModel->_get_where([
		...
		$this->data['template'] = 'backend/display/catalogue/index';
	...
	public function create(){
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($resultid > 0){
					...
					$this->nestedsetbie->Action();
 					...
 					return redirect()->to(BASE_URL.'backend/display/catalogue/create');
		 		}
		...
		$this->data['template'] = 'backend/display/catalogue/create';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function update($id = 0){
		...
		$this->data[$this->data['module']] = $this->AutoloadModel->_get_where([
			'select' => 'tb1.id, tb2.title, tb2.canonical, tb2.description, tb2.content, tb2.meta_title, tb2.meta_description, tb1.parentid, tb1.image,  tb1.publish',
		...
		if(!isset($this->data[$this->data['module']]) || is_array($this->data[$this->data['module']]) == false || count($this->data[$this->data['module']]) == 0){
			$session->setFlashdata('message-danger', 'Nhóm Bài Viết không tồn tại');
 			return redirect()->to(BASE_URL.'backend/display/catalogue/index');
		}
		...
		if($this->request->getMethod() == 'post'){
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])){
		 		...
		 		if($flag > 0){
		 			$flag = $this->AutoloadModel->_update([
			 			'table' => 'display_translate',
			 			'where' => ['objectid' => $id,'module' => 'display_catalogue', 'language' => $this->currentLanguage()],
			 			'data' => $updateLanguage
			 		]);
		...
		$this->data['template'] = 'backend/display/catalogue/update';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	public function delete($id = 0){
		...
		$this->data['template'] = 'backend/display/catalogue/delete';
		return view('backend/dashboard/layout/home', $this->data);
	}
	...
	private function detect_language(){
		...
		if(isset($languageList) && is_array($languageList) && count($languageList)){
			foreach($languageList as $key => $val){
				$select = $select.'(SELECT COUNT(objectid) FROM display_translate WHERE display_translate.objectid = tb1.id AND display_translate.language = "'.$val['canonical'].'") as '.$val['canonical'].'_detect, ';
				$i++;
			}	
		}
