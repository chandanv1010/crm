...
class Hosting extends BaseController{
	...
	public function update($id = 0){
		$this->data[$this->data['module']] = $this->AutoloadModel->_get_where([
			'select' => 'id, contract_code, customerid, userid, price, fullname, phone, email, address, ip_vps, service, date_start, date_end, timeline, total, date_sign, contract_file, description, domain',
	...
	public function extend($id = 0){
		$this->data[$this->data['module']] = $this->AutoloadModel->_get_where([
			'select' => 'id, contract_code, customerid, userid, fullname, phone, email, address, ip_vps, service, date_end, domain',
			'table' => $this->data['module'],
			'where' => ['id' => $id,'deleted_at' => 0],
		]);
	...
	public function upgrade($id = 0){
		...
		$this->data[$this->data['module']] = $this->AutoloadModel->_get_where([
			'select' => 'tb1.id, tb1.contract_code, tb1.customerid, tb1.userid, tb1.fullname, tb1.phone, tb1.email, tb1.address, tb1.ip_vps, tb1.service, tb1.date_start, tb1.date_end, tb1.domain, tb2.title as service_name, tb1.price, tb3.ip as ip_name, (SELECT TIMESTAMPDIFF(DAY,NOW(),date_end)) as day',
			'table' =>$this->data['module'].' as tb1',
