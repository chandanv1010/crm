    ...
    HT.price = () =>{
    	$(document).on('change','select[name=domainid]',function(){
    		let _this = $(this);
    		...
    		if(method == 'update' && count == 0){
	    		...
	    		let domainid = _this.val();
	    		$('select[name=price-hidden]').val(domainid).trigger('change');
    ...
    HT.money = () =>{
    	$(document).on('change', 'input[name=price], select[name=timeline]', function(){
    		...
    		$('input[name=total]').val(price*(timeline/12)).trigger('change');
    	});
    };
    ...
    HT.showContract = () => {
		$(document).on('click','.show-infor', function(){
			...
			if($('.show-ojectid-'+id+'').length){
				...
				$('.domain-infor').remove();
				let formURL = 'ajax/ContractDomain/getInforContract';
				clearTimeout(time);
	...
	HT.renderHostingInfor = (infor, id) =>{
		let html = '';
		 if(infor.length > 0 ){
		 	html= html + '<tr class="domain-infor show-ojectid-'+id+'">'
                    html= html + '<td>Lần</td>'
                    html= html + '<td colspan="2"> Tên miền</td>'
        	...
        	infor.forEach(function(item, index, array){
                html= html + '<tr class="domain-infor show-ojectid-'+item.objectid+'">'
                    html= html + '<td>'+(index + 1)+'</td>'
                    html= html + '<td colspan="2">'+item.domain+item.domain_title+'</td>'
			...
			html= html + '<tr class="domain-infor show-ojectid-'+id+'">'
				html = html + '<td class="text-danger" colspan="100%">Domain chưa được gian hạn lần nào!</div>';
			html= html + '</tr>'
		}
		return html;
	}
 	 	...
 	 	if(saveButton.elExists){
 	 		$('.object-form').on('submit', function(){
				let _this = $(this);
 	 			...
 	 			let domainid = $('select[name=domainid]').val();
			...
			setTimeout(function(){
				...
				$.post(formURL,{id:postid},function(data){
					...
					if(response.code == 10){
						...
						var domainid = HT.renderFormSelect(response.data.domainid, 'domainid');
						var price_sell = HT.renderFormSelect(response.data.price_sell, 'price-hidden');
						var price_extend = HT.renderFormSelect(response.data.price_extend, 'price-hidden');
						let dataUpdate = response.data.selectUpdate;

						if($('#id').val() != 0){
							let method = $('input[name=method]').val();
							let userid = dataUpdate.userid;
							let _domainid = dataUpdate.domainid;
							let _timeline = dataUpdate.timeline;

							$('.form-staff').append(staff);
							$('.form-domain').append(domainid);
							if($('input[name=method]').val() == 'extend'){
								$('.form-price-hidden').append(price_extend);
							}else{
								$('.form-price-hidden').append(price_sell);
							}
							$('.form-timeline').append(timeline);

							$('.form-staff').find('.select2').val(userid).trigger('change');
							$('select[name=domainid]').val(_domainid).trigger('change');
